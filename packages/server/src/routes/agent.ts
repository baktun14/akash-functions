...
function buildSystemPrompt(context: AgentChatContext, models: readonly string[]): string {
  ...
  const base =
    ...
    'Guidelines:\n' +
    '- You may import any npm package — dependencies are installed at deploy time via `bun install`. Pin versions inline in the import when it matters, Railway-style: `import { Hono } from "hono@4"`. Use Hono for HTTP routing when you want middleware/validators/typed params; Bun\'s built-in `Bun.serve({ routes })` is a fine zero-dep alternative for simple APIs.\n' +
    '- For scheduled tasks, PREFER `Bun.cron("0 * * * *", async () => { ... })` over the `croner` package — it ships with Bun, takes standard 5-field cron expressions, and needs no install. Pair it with a tiny `Bun.serve` that returns `new Response("ok")` so the container keeps an HTTP listener and doesn\'t exit.\n' +
